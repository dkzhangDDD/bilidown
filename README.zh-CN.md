...
把 B 站视频变成一份可学习、可检索、可保存的资料。扩展会读取视频已有字幕，并在 Chrome 侧边栏中提供字幕浏览、AI 概览（DeepSeek / MiniMax）、完整演讲总结笔记、章节、关键观点、选中文本解释、时间戳跳转和本地笔记。
...
6. 在设置页选择 AI 模型（DeepSeek / MiniMax / 自定义），填入对应的 API Key 并保存。
...
AI API Key、笔记和缓存只保存在当前 Chrome 配置中。字幕从 B 站接口读取；使用 AI 功能时，相关字幕和视频信息会发送给你选择的 AI 服务（DeepSeek 或 MiniMax）。不要把 API Key 写进源码、提交记录、截图或聊天。
