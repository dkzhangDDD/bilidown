...
bilidown 会直接显示在哔哩哔哩视频页面中。配置阿里云百炼 API Key 后，默认使用 Fun-ASR 识别当前视频音轨；未配置百炼 Key 时，才通过 B 站 WBI 播放器接口读取原生字幕。获得文字后，再由你选择的 AI 服务（DeepSeek / MiniMax，或任意 OpenAI 兼容服务）生成中文概览、章节时间线和关键观点。
